...
describe(
  "Search and buy products as authorized user",
  { tags: "@smoke" },
  () => {
    ...
    it("should be able to buy selected product", () => {
      ...
      shoppingCartPage
        .findProductName(searchedProduct)
        .should("contain.text", searchedProduct);
      shoppingCartPage
        .getProductQuantityByName(searchedProduct)
        .should("have.value", productQuantity);
      shoppingCartPage.clickOnCheckoutButton();
      checkoutPage
        .findProductByName(searchedProduct)
        .should("contain.text", searchedProduct);
      checkoutPage
        .getProductQuantityByName(searchedProduct)
        .should("have.value", productQuantity);
    ...
    it("should be able to buy more than one product", () => {
      ...
      for (const product of products) {
        ...
        productsPage.clickOnViewCartButton();
        shoppingCartPage
          .findProductName(product.name)
          .should("contain.text", product.name);
        shoppingCartPage
          .getProductQuantityByName(product.name)
          .should("have.value", productQuantity);
      ...
      checkoutPage.checkTermsAndConditions();
      checkoutPage.clickOnContinueButton();
      confirmOrderPage.assertConfirmationOrderPageProducts(
        products,
        productQuantity
      );
      confirmOrderPage.clickOnConfirmOrderButton();
